...
/*   Updated: 2021/03/19 16:20:20 by pvivian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	public:
		...
		// typedef typename ft::reverse_map_iterator<value_type>			reverse_iterator;
	...
	// *************** Iterators ***************
		...
		begin(void)
		{
			...
			iterator beg(this->_tree.min(root));
			return beg;
		}
		...
		end(void) 
		{
			...
			iterator ending(this->_tree.max(root));
			return ++ending;
		}
		...
		rbegin()
		{
			...
			treeNode*  = this->_tree.getRoot();
			reverse_iterator rbeg(this->_tree.max(root));
			return rbeg;
		}
		...
		{
			treeNode*  = this->_tree.getRoot();
			reverse_iterator rending(this->_tree.min(root));
			return --ending;
		}
	...
	// *************** Capacity ***************
		...
		size_type
		size(void) const { return return this->_tree.size(); }
	...
	// *************** Element access ***************
		...
		{
			tree_type::node* node = this->_tree._findNode(k);
			if (node == NULL)
	...
	// *************** Modifiers ***************
		...
		insert(const value_type& val)
		{
			...
			bool res = false;
			tree_type::node* node = this->_tree._findNode(val.first);
	
			if (node == NULL)
			{
				node = this->_tree.insert(val);
				res = true;
			}
